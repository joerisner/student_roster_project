...
// Parse the studentDataString (from main) by comma delimiters, then call the Add function to create pointers for each student object based on the parsed variables.
...
// "new" does NOT create a new Student object. It creates a new POINTER to a student object because classRosterArray is an array of pointers, not objects.
// increment craIndex so that classRosterArray at index 0 doesn't just get created 5 times.
...
// Loop through classRosterArray and call Student::Print() on each student object.
...
// Print a student's average number of days in three courses. Takes an argument of studentID.
...
// Prints invalid emails. Valid email should include an ('@') symbol, a period ('.') and should not include a space (' ').
...
// Print student information by degreeProgram
...
// Remove student from classRosterArray by studentID. If studentID does not exist, print an error indicating student was not found.
    ...
    for (int i = 0; i < ARR_SIZE; i++) {
        if (classRosterArray[i] != NULL) {
            ...
            if ( id == studentID) {
                ...
                classRosterArray[i] = nullptr; // The pointer at this index must be set to NULL to deallocate the memory
